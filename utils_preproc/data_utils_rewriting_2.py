...
def encode(tokenizer, max_seq_length, tokens_a, tokens_b=None):
    ...
    input_ids = tokenizer.convert_tokens_to_ids(tokens) #先append符号，再ids, 而xlnet是先转id再append符号
...
def process_single_example(mode, example, max_seq_length_x, max_seq_length_y, tokenizer):
    ...
    yy = tokenizer.tokenize(yy1 + ' ' + yy2 + ' ' + yy3)
    ...
    input_ids_y, input_mask_y, segment_ids_y, _ = encode(tokenizer,max_seq_length_x, y)
    input_ids_yy, input_mask_yy, segment_ids_yy, _ = encode(tokenizer,max_seq_length_x, yy)
    ...
    feature_x = {
        ...
        "input_ids_yy": input_ids_yy,
        "input_mask_yy": input_mask_yy,
        "segment_ids_yy": segment_ids_yy,
...
def file_based_convert_examples_to_features_v2(mode,
    ...
    for (_, example) in enumerate(examples):
        ...
        features["segment_ids_yy"] = _create_int_feature(fea_x["segment_ids_yy"])

        features["input_ids_y"] = _create_int_feature(fea_x["input_ids_y"])
        features["input_mask_y"] = _create_int_feature(fea_x["input_mask_y"])
        features["segment_ids_y"] = _create_int_feature(fea_x["segment_ids_y"])
