...
__all__ = [
    "AdjMultiheadAttentionEncoder"
]

class AdjMultiheadAttentionEncoder(EncoderBase):
    ...
    def default_hparams():
        ...
        "initializer": dict, optional
            ...
            See :func:`~texar.tf.tf.core.get_initializer` for details.
    ...
    def _build(self, queries, memory, memory_attention_bias=None,
        ...
        Args:
            ...
            mode (optional): A tensor taking value in
                ...
                If `None` (default), :func:`texar.tf.tf.global_mode` is used.
        ...
        with tf.variable_scope(self.variable_scope):
            ...
            weights = tf.layers.dropout(weights,
                                        ...
                                        training=is_train_mode(mode)) #[batch, num_heads, maxlen, maxlen]
            weights = tf.transpose(weights, [0,2,3,1]) # `[batch, seq_len, seq_len, num_heads]`

            weights = self.O_dense(weights) # `[batch, seq_len, seq_len, 1]`
            weights = tf.squeeze(weights, 3)
            '''
    ...
    def _combine_heads(self, x):
        """
        Args:
            x: A Tensor of shape `[batch, num_heads, seq_len, seq_len]`

        Returns:
            A Tensor of shape `[batch, seq_len, num_heads * seq_len]`
        """
        t = tf.transpose(x, [0, 2, 1, 3]) #[batch, seq_len, num_heads, seq_len]
