...
from texar.tf.modules import WordEmbedder
from texar.tf.modules import TransformerEncoder
from texar.tf.modules import MLPTransformConnector
from texar.tf.modules import BidirectionalRNNEncoder
from texar.tf.modules import UnidirectionalRNNEncoder
...
class EvolveGTAE(object):
    ...
    def _prepare_inputs(self, inputs, vocab, ctx_maxSeqLen, lr):
        ...
        self.x1x2_mask = inputs['text_input_mask_x1x2']
        self.x1xx2 = inputs['text_input_ids_x1xx2']
        self.x1xx2_mask = inputs['text_input_mask_x1xx2']
        self.x1x2yx1xx2_ids = inputs['text_input_ids_x1x2ysx1xx2']     # x1 + ' ' + x2 + ' ' + y + ' | ' + x1 + ' ' + xx2 + ' ' 
        self.x1x2yx1xx2_mask = inputs['text_input_mask_x1x2ysx1xx2']
        self.x1x2yx1xx2yy_ids = inputs['text_input_ids_x1x2ysx1xx2yy'] # x1 + ' ' + x2 + ' ' + y + ' | ' + x1 + ' ' + xx2 + ' '
        self.x1x2yx1xx2yy_mask = inputs['text_input_mask_x1x2ysx1xx2yy'] # x1 + ' ' + x2 + ' ' + y + ' | ' + x1 + ' ' + xx2 + ' ' 
        ...
        self.text_mask_y1 = inputs['text_input_mask_y1']    # [batch, maxlen1]
        self.text_mask_yy1 = inputs['text_input_mask_yy1']  # [batch, maxlen1]
        self.text_mask_y2 = inputs['text_input_mask_y2']    # [batch, maxlen2]
        self.text_mask_yy2 = inputs['text_input_mask_yy2']  # [batch, maxlen2]
        self.text_mask_y3 = inputs['text_input_mask_y3']    # [batch, maxlen3]
        self.text_mask_yy3 = inputs['text_input_mask_yy3']  # [batch, maxlen3]
    ...
    def _prepare_modules(self):
        ...
        # encode ctx
        self.xlnet_encoder = XLNetEncoder(hparams=self._hparams.xlnet_encoder)
        ...
        self.rephrase_decoder = DynamicAttentionRNNDecoder(
            memory_sequence_length = self.sequence_length_y1-1,                                                        ## use yy1's truth length ###check?
    ...
    def _build_ctx_encoder(self):
        ...
        """
        # ctx
        self.ctx_embd = self.xlnet_encoder(
            token_ids =self.x1x2yx1xx2_ids,
            input_mask = tf.to_float(self.x1x2yx1xx2_mask)
        )[0]                                                                                                             # [batch, maxlen, dim]                                                                           
    ...
    def _build_cft_graph_y_encoder(self, y_truth_len, yy_text_ids, yy_truth_len):
        ...
        """         
        yy_pred_output = tf.nn.relu(self.downmlp(
