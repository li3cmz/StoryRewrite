...
class TFRecordDataNumpyDecoder(TFRecordDataDecoder):
    ...
    See 'texar.tf.data.data_decoders.TFRecordDataDecoder' for details
    """
    ...
    def _decode_numpy_ndarray_str_byte(self,
        ...
        raw_shape=tf.cast(tf.sqrt(tf.cast(tf.shape(numpy_ndarray), tf.float32)), tf.int32) #[]
        minus_shape = shape[0]-raw_shape[0] #int
        numpy_ndarray = tf.reshape(numpy_ndarray, [raw_shape[0],raw_shape[0]])
        numpy_ndarray = tf.pad(numpy_ndarray, [[0,minus_shape],[0,minus_shape]],"CONSTANT")
